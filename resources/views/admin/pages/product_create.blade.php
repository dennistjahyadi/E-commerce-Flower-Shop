                            ...
                            <div class="card">
                                <div class="card-body card-block">
                                    <form method="post" action="/admin/product-create" enctype="multipart/form-data">
                                                ...
                                                <select name="category" class="form-control">
                                                    ...
                                                    @if(isset($categories) &&count($categories) > 0)
                                                        @foreach($categories as $obj)
                                                            <option value="{{ $obj->id }}" {{ old("category") == $obj->id  ? "selected":"" }}>{{ $obj->name }}</option>
                                        ...
                                        <div class="form-group col-xs-6">
                                            <div class="form-group col-xs-8">
                                                <label>Default Cover Image <span class="link">*</span></label>
                                                <p><input type="file" name="image_1" accept="image/*"></p>
                                                <div style="line-height:1;margin-top:10px;"><i class="product-detail-info">
                                                        It will be the default picture of this product.</i></div>
                                                <div class="form-error">
                                                    {!! $errors->first('image_1', '<p class="help-block" style="color: red;">Please upload an default image</p>') !!}
                                                </div>
                                            </div>
                                            <div class="col-xs-12">
                                            </div>
